...
public:
	...
	void dfs(int start, int64_t sum, int target, vector<int> &ans, vector<vector<int>> &result, const vector<int> &nums)
	{
		if (ans.size() == 4)
		{
			if (sum == target)
		...
		if (ans.size() + nums.size() - start < 4)
		{
			return;
		}

		for (int i = start; i < nums.size() && ans.size() < 4; ) //如果不是连续，就需要for遍历，如果是连续，则不用for
		{
			ans.push_back(nums[i]);
			dfs(i + 1, sum + nums[i], target, ans, result, nums);
			ans.pop_back();
	...
	vector<vector<int>> fourSum(vector<int>& nums, int target)
	{
		...
		dfs(0, 0, target, ans, result, nums);
